...
use thockflow::keymap::{KeymapData, PhysicalKey, Layer, generate_svg, parse_raw_bindings};
...
fn parse_keymap(content: &str) -> Result<KeymapData> {
    ...
    if root_node.has_error() {
        fn find_error(node: tree_sitter::Node, _source: &[u8], pos: &mut String) {
            if node.has_error() {
                if node.kind() == "ERROR" {
                    *pos = format!("Parse error at line {}, column {}", node.start_position().row + 1, node.start_position().column + 1);
                } else {
    ...
    if physical_layout.is_empty() && !layers.is_empty() {
        ...
        if !matches.is_empty() {
            ...
            eprintln!("Matched layout: {} from {}", matched_layout.name, matched_layout.source_file);
            physical_layout = matched_layout.keys.iter().map(|k| PhysicalKey {
                width: k.width, height: k.height, x: k.x, y: k.y,
                rotation: k.rotation, rx: k.rx, ry: k.ry,
            }).collect();
        }
    }

    if physical_layout.is_empty() {
        return Err(anyhow::anyhow!("Missing physical layout and no match found for {} keys", layers.get(0).map_or(0, |l| l.bindings.len())));
    }
    if layers.is_empty() {
