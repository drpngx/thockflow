...
pub fn align_incremental(quote: &str, input: &str) -> Vec<(EditOp, Option<char>, Option<char>)> {
    ...
    while i > 0 || j > 0 {
        ...
        let char_match = if i > 0 && j > 0 {
            input_chars[i - 1] == quote_chars[j - 1]
        } else {
            false
        };
...
mod tests {
    ...
    fn test_skip_char() {
        ...
        let skips = res
            .iter()
            .filter(|(op, _, input_char)| *op == EditOp::Substitute && input_char.is_none())
            .count();
        assert_eq!(skips, 1);
    }
    ...
    fn test_long_error_string_issue() {
        ...
        assert_eq!(
            ...
            "Should match 'the brown' correctly"
        );
    }
